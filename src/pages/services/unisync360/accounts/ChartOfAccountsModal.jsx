...
export const ChartOfAccountsModal = ({ show, selectedObj, onSuccess, onClose }) => {
    ...
    const handleSubmit = async (values, { setErrors }) => {
        try {
            ...
            if (result) {
                showToast("success", `Account ${selectedObj?.uid ? 'Updated' : 'Created'} Successfully`);
                if (onClose) onClose();
    ...
    return (
        <GlobalModal
            show={show}
            onClose={onClose}
            title={<><i className="bx bx-wallet me-2"></i>{selectedObj?.uid ? "Update Account" : "Create New Account"}</>}
            size="lg"

...
export const getDocumentRequirements = async ({ search = "", pagination = {}, filters = {} } = {}) => {
    try {
        const params = new URLSearchParams();
        if (search) params.append("search", search);
        if (pagination.page) params.append("page", pagination.page);
        ...
        const response = await api.get(`${API_URL}/unisync360-applications/document-requirements/?${params.toString()}`);
        return response.data;
    } catch (error) {
...
export const getDocumentRequirement = async (uid) => {
    try {
        const response = await api.get(`${API_URL}/unisync360-applications/document-requirements/${uid}/`);
        return response.data;
    } catch (error) {
...
export const createDocumentRequirement = async (data) => {
    try {
        const response = await api.post(`${API_URL}/unisync360-applications/document-requirements/`, data);
        return response.data;
    } catch (error) {
...
export const updateDocumentRequirement = async (uid, data) => {
    try {
        const response = await api.patch(`${API_URL}/unisync360-applications/document-requirements/${uid}/`, data);
        return response.data;
    } catch (error) {
...
export const deleteDocumentRequirement = async (uid) => {
    try {
        const response = await api.delete(`${API_URL}/unisync360-applications/document-requirements/${uid}/`);
        return response.data;
    } catch (error) {
...
// Document Type options (matches backend DOCUMENT_TYPES)
export const DOCUMENT_TYPE_OPTIONS = [
    { value: 'academic', label: 'Academic', color: 'primary' },
    { value: 'identity', label: 'Identity', color: 'info' },
    { value: 'financial', label: 'Financial', color: 'success' },

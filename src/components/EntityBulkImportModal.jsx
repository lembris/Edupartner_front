...
// API functions for entity bulk import
const EntityBulkImportAPI = {
    ...
    getJobs: async (importType, queryParams = {}) => {
        const url = importType
            ? `${ENTITY_IMPORT_URL}/jobs/${importType}/`
            : `${ENTITY_IMPORT_URL}/jobs/`;
    ...
    deleteJob: async (jobId) => {
        const response = await api.delete(`${ENTITY_IMPORT_URL}/jobs/detail/${jobId}/`);
        return response.data;
    },
...
export const EntityBulkImportModal = ({ importType, onSuccess, onClose }) => {
    ...
    const renderStep3 = () => (
        <div className="p-3">
            {/* Results Summary */}
            <div className={`alert ${currentJob?.status === 'completed' ? 'alert-success' : currentJob?.status === 'failed' ? 'alert-danger' : 'alert-warning'} mb-4`}>
                <div className="d-flex align-items-center">
                    <i className={`bx ${currentJob?.status === 'completed' ? 'bx-check-circle' : currentJob?.status === 'failed' ? 'bx-x-circle' : 'bx-error'} fs-3 me-3`}></i>
                    <div>
                        <h5 className="mb-1">
                            Import {currentJob?.status === 'completed' ? 'Completed' : currentJob?.status === 'failed' ? 'Failed' : 'Partially Completed'}
                        </h5>
                        ...
                        <div className="d-flex justify-content-between position-relative">
                            ...
                            {['Upload', 'Processing', 'Results'].map((label, idx) => (
                                <div key={idx} className="text-center position-relative" style={{ zIndex: 1 }}>
                                    <div
                                        className={`rounded-circle d-inline-flex align-items-center justify-content-center ${step > idx + 1 ? 'bg-success' : step === idx + 1 ? 'bg-primary' : 'bg-secondary'
                                            }`}
...
export default EntityBulkImportModal;
